...
/**
 * TabBarItem API for the iOS preview Environment.
 * 
 * HTML Button instead of tab
 */
...
WL.TabBarItem = WLJSX.Class.create({
    ...
    initialize : function(id, title, callback, tabDiv) {
	this.__id = id;
	__button = document.createElement("button");
	var buttonTitle = document.createTextNode(title);
	__button.appendChild(buttonTitle);
	__button.id = 'WLButton_' + id;
	__button.onclick = function() {
	    callback();
	};
	tabDiv.appendChild(__button);
    },

    setEnabled : function(isEnabled) {
	WL.Validators.validateArguments(['boolean'], arguments, 'WL.TabBarItem.setEnabled');
	__button.disabled = !isEnabled;
    },
...
/**
 * TabBar API for the iOS preview Environment.
 * 
 * HTML Button instead of tab
 */
__WLTabBar = function() {
	...
	var item = new WL.TabBarItem(id, title, callback, tabDiv);
	return item;
    };
	...
	var tabDivLength = tabDiv.childNodes.length;
	for ( var x = tabDivLength - 1; x >= 0; x--) {
	    tabDiv.removeChild(tabDiv.childNodes[x]);
	}
	items.length=0;
    };
	...
	WL.Validators.validateArguments(['boolean'], arguments, 'WL.TabBar.setEnabled');
	for ( var x = 0; x < tabDiv.childNodes.length; x++) {
	    tabDiv.childNodes[x].disabled = !isEnabled;
	}
    };
