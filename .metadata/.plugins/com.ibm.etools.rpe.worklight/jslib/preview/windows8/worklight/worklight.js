...
/* This file is for Windows Universal only. We need to read the wlclient.properties file 
...
function __loadStaticAppProps(lines){
    ...
    if (window.clientInformation.userAgent.indexOf('Windows Phone 8.1') > -1) {
        WL.StaticAppProps.WORKLIGHT_ROOT_URL = rootUrl + WL.StaticAppProps.WORKLIGHT_ROOT_URL_WPUNI;
        WL.StaticAppProps.APP_VERSION = WL.StaticAppProps.APP_VERSION_WPUNI;
        WL.StaticAppProps.ENVIRONMENT = WL.StaticAppProps.ENVIRONMENT_WPUNI;
    } else {
        WL.StaticAppProps.WORKLIGHT_ROOT_URL = rootUrl + WL.StaticAppProps.WORKLIGHT_ROOT_URL;
    }
}

var url = new Windows.Foundation.Uri('ms-appx:///wlclient.properties');
Windows.Storage.StorageFile.getFileFromApplicationUriAsync(url).then(function (file) {
    ...
    function (err){
		console.error('Unable to read wlclient.properties. Ensure that wlclient.properties has the right permissions. ' + JSON.stringify(err));
    });
},
function(err){
	console.error('Unable to load wlclient.properties. Ensure that wlclient.properties is available.' + JSON.stringify(err));
}
);
...
/**
 * ================================================================= 
 * Source file taken from :: busyindicator.windows8.js
 * ================================================================= 
 */
...
WLAuthorizationManager.__deleteTokens = function () {
	var dfd = WLJQ.Deferred();
...
WLAuthorizationManager.__getSignedClientId = function () {
	...
	WLAuthorizationManager.__signCSR(payloadJSON, keyPair).then(
	        function(data) {
	        	this.signedClient = data;
	        	if (this.signedClient === null || typeof(this.signedClient) === 'undefined') {
	        		WL.Logger.error("signedClient is null or undefined");
	        		dfd.reject();
	        	} else {
		            dfd.resolve(this.signedClient);
		        }
	          },
	          function(error) {
	            WL.Logger.debug('signedClient request failed with error: ' + JSON.stringify(error));
	            dfd.reject(error);
	          }
	        );
...
WLAuthorizationManager.signedClientId = null;
...
WL.Client.Push.__isDeviceSupportPush = function() {
	return false;
};
